...
import { Users, AlertTriangle, TrendingUp, Filter, X } from 'lucide-react';
import { ResponsiveContainer, BarChart, Bar, XAxis, YAxis, Tooltip, CartesianGrid } from 'recharts';
import { useNotification } from '@/lib/NotificationContext';

export default function MobileDebtsReport() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetch(`/api/reports/debts?${params}`)
      .then(r => r.json())
      .then(d => { setData(d); setLoading(false); })
      .catch(() => { error('Xatolik', 'Qarzlar hisobotini yuklashda xato'); setLoading(false); });
  }, [dateFrom, dateTo]);
  ...
  return (
    <div className="w-full min-h-screen pb-28">
      <MobileHeader title="Qarzlar Hisoboti" backHref="/mobile/reports"
        rightAction={
      ...
      <div className="px-6 mt-2 space-y-4">
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-100 dark:border-slate-800">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[10px] font-bold text-slate-500 uppercase">Qarzdorlar</span>
            </div>
            <p className="text-xl font-black text-slate-900 dark:text-white">{loading ? '...' : (summary.totalDebtors || 0).toLocaleString()}</p>
          </div>
          <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-100 dark:border-slate-800">
            <div className="flex items-center gap-2 mb-2">
              <AlertTriangle size={14} className="text-red-500" />
              <span className="text-[10px] font-bold text-slate-500 uppercase">Jami qarz</span>
            </div>
            <p className="text-xl font-black text-red-600">${loading ? '...' : (summary.totalDebt || 0).toLocaleString()}</p>
          </div>
          <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-100 dark:border-slate-800">
            <div className="flex items-center gap-2 mb-2">
              <TrendingUp size={14} className="text-emerald-500" />
              <span className="text-[10px] font-bold text-slate-500 uppercase">Kreditlar</span>
            </div>
            <p className="text-xl font-black text-slate-900 dark:text-white">${loading ? '...' : (summary.totalCredits || 0).toLocaleString()}</p>
          </div>
          <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-100 dark:border-slate-800">
            <div className="flex items-center gap-2 mb-2">
              <Users size={14} className="text-blue-500" />
              <span className="text-[10px] font-bold text-slate-500 uppercase">Kreditorlar</span>
            </div>
            <p className="text-xl font-black text-slate-900 dark:text-white">{loading ? '...' : (summary.totalCreditors || 0).toLocaleString()}</p>
          </div>
        </div>

        <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-100 dark:border-slate-800">
          <h3 className="text-sm font-bold text-slate-900 dark:text-white mb-3">Qarzlar dinamikasi</h3>
          {loading ? (
            ...
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={chartData}>
                ...
                <Tooltip />
                <Bar dataKey="qarz" fill="#f59e0b" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </div>
        ...
        <div>
          <h3 className="text-sm font-bold text-slate-900 dark:text-white mb-3">Qarzdorlar ro{'\u2019'}yxati</h3>
              ...
              <div key={i} className="bg-white dark:bg-slate-900 p-3.5 rounded-xl border border-slate-100 dark:border-slate-800">
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500">{String(row.phone || '-')}</p>
                  </div>
                  <p className="text-sm font-black text-red-600">${String(row.debt || '0')}</p>
                </div>
              </div>
            ))}
          </div>
