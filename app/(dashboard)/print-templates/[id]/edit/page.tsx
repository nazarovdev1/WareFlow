...
export default function EditTemplatePage() {
  ...
  const [mode, setMode] = useState<'visual' | 'advanced'>('advanced');
  ...
  const loadTemplate = async (id: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        error('Xatolik', 'Shablon topilmadi');
        router.push('/print-templates');
      }
    } catch {
      error('Xatolik', 'Yuklashda xatolik');
    } finally {
      setLoading(false);
    }
  };
  ...
  const generateHTML = useCallback((isPreview = false) => {
    ...
    const docTypeLabel = DOC_TYPE_LABELS[template.type] || 'HUJJAT';
    ...
    if (isThermal) {
      const width = template.paperSize === '80mm' ? '72mm' : '50mm';
      return `<!DOCTYPE html>
<html>
<head>
  <style>
    @media print {
      @page { size: ${template.paperSize} auto; margin: 0; }
...
<head>
  <style>
    @media print {
      @page { size: ${template.paperSize} ${template.orientation}; margin: 10mm; }
    }
...
<body>
  ...
  }, [visualData, template.type, template.paperSize, template.orientation]);

  useEffect(() => {
    if (mode === 'visual' && !loading) {
      setTemplate(prev => ({ ...prev, content: generateHTML(false) }));
    }
  }, [visualData, mode, template.type, template.paperSize, loading, generateHTML]);
    ...
    return (
      <div className="w-full min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-950">
        <div className="flex flex-col items-center gap-4">
          ...
          <div className="text-slate-400 font-bold uppercase tracking-widest text-xs">Yuklanmoqda...</div>
        </div>
      </div>
    );
  }
  ...
  const isThermal = template.paperSize === '80mm' || template.paperSize === '58mm';
  const previewWidth = isThermal ? 'auto' : template.paperSize === 'A4' ? '210mm' : '148mm';
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-black text-slate-900 dark:text-white">Shablonni tahrirlash</h1>
              <p className="text-xs text-slate-500 font-bold uppercase tracking-wider">{template.name}</p>
            </div>
          </div>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-[32px] shadow-sm border border-slate-200/60 dark:border-slate-800/60 overflow-hidden">
              <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-800/60 flex items-center justify-between bg-slate-50/50 dark:bg-slate-800/20">
                <div className="flex items-center gap-2 text-indigo-600 dark:text-indigo-400">
                  <Palette size={18} />
                  <span className="text-sm font-black uppercase tracking-widest">Sozlamalar</span>
                </div>
              </div>
              ...
              <div className="p-8 space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      value={template.type}
                      onChange={e => setTemplate({ ...template, type: e.target.value as PrintTemplate['type'] })}
                ...
                <div className="flex-1 p-8 bg-slate-200/50 dark:bg-slate-950/50 overflow-auto flex justify-center">
                  {isThermal ? (
                    <div className="bg-white shadow-2xl" style={{ width: 'auto', maxWidth: '100%' }}>
                      <div
                        style={{
                          width: template.paperSize === '80mm' ? '80mm' : '58mm',
                          minHeight: 'auto',
                          fontSize: '10pt',
                          fontFamily: "'Courier New', monospace",
                        }}
                        dangerouslySetInnerHTML={{ __html: mode === 'visual' ? generateHTML(true) : template.content }}
                      />
                    </div>
                  ) : (
                    <div 
                    ...
                    >
                      <div dangerouslySetInnerHTML={{ __html: mode === 'visual' ? generateHTML(true) : template.content }} />
                    </div>
                  )}
                </div>
              </div>
